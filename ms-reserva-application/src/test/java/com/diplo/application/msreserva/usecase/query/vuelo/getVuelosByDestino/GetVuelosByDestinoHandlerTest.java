...
class GetVuelosByDestinoHandlerTest {
	...
	@Mock IVueloRepository _vueloRepositoryTest;
	@Mock GetVuelosByDestinoQuery getVuelosByDestinoQueryTest;
	...
	void init() throws Exception {
		...
		getVuelosByDestinoHandlerTest = new GetVuelosByDestinoHandler(_vueloRepositoryTest);
		...
		vueloTest = new Vuelo(vueloIdTest, numeroVueloTest, _DestinoTest, cantidadAsientoDisponibleTest);
		
		listaVueloTest = new ArrayList<Vuelo>();
		listaVueloTest.add(vueloTest);
		
		when(getVuelosByDestinoQueryTest.getDestino()).thenReturn(_DestinoTest.getNombreDestino());
		when(_vueloRepositoryTest.GetVuelosByDestino(any())).thenReturn(CompletableFuture.completedFuture(listaVueloTest));
		
	}
	
	@Test
	void test() throws InterruptedException, ExecutionException {
		Future<List<VueloDTO>> resultado = getVuelosByDestinoHandlerTest.Handle(getVuelosByDestinoQueryTest);
		
		assertNotNull(resultado);
		assertEquals(vueloIdTest.toString(), resultado.get().get(0).getVueloId());
	}
