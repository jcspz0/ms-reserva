...
class GetReservaByIdHandlerTest {
	...
	@Mock GetReservaByIdQuery getReservaByIdQueryTest;
	...
	void init() throws Exception {
		...
		getReservaByIdHandlerTest = new GetReservaByIdHandler(_reservaRepositoryTest);
		
		when(_reservaRepositoryTest.FindByIdAsync(any())).thenReturn(CompletableFuture.completedFuture(reservaTest));
	...
	void GetReservaByIdHandler() throws Exception{
		Future<ReservaDTO> resultado = getReservaByIdHandlerTest.Handle(getReservaByIdQueryTest);
		
		assertNotNull(resultado);
		assertEquals(reservaTest.getId().toString(), resultado.get().getReservaId());
	}
