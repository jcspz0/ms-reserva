...
class GetReservasByHoraAndEstadoHandlerTest {
	...
	@Mock GetReservasByHoraAndEstadoQuery getReservasByHoraAndEstadoQueryTest;
	...
	void init() throws Exception {
		...
		getReservasByHoraAndEstadoHandlerTest = new GetReservasByHoraAndEstadoHandler(_reservaRepositoryTest);
		...
		when(getReservasByHoraAndEstadoQueryTest.getHora()).thenReturn(HoraTest);
		when(_reservaRepositoryTest.GetReservasByHoraAndEstado(any(), any())).thenReturn(CompletableFuture.completedFuture(listaReservaTest));
		
	}
	
	@Test
	void GetReservasByHoraAndEstado() throws InterruptedException, ExecutionException {
		Future<List<ReservaDTO>> resultado = getReservasByHoraAndEstadoHandlerTest.Handle(getReservasByHoraAndEstadoQueryTest);
		
		assertNotNull(resultado);
		assertEquals(resultado.get().get(0).getReservaId(), reservaIdTest.toString());
	}
