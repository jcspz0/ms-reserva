...
public class GetReservasByHoraAndEstadoHandler implements IRequestHandler<GetReservasByHoraAndEstadoQuery, List<ReservaDTO>>{

	
	private final IReservaRepository _reservaRepository;
	
	public GetReservasByHoraAndEstadoHandler(IReservaRepository _reservaRepository) {
		super();
		this._reservaRepository = _reservaRepository;
	}

	@Override
	public Future<List<ReservaDTO>> Handle(GetReservasByHoraAndEstadoQuery request) {
		
		try {
			String str = request.getHora();
			DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
			LocalDateTime dateTime = LocalDateTime.parse(str, formatter);
			CompletableFuture<List<Reserva>> future =  (CompletableFuture<List<Reserva>>) _reservaRepository.GetReservasByHoraAndEstado(dateTime, request.getEstado());
