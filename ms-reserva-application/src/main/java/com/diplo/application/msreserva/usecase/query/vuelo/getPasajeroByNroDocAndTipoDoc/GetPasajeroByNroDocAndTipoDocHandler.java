...
public class GetPasajeroByNroDocAndTipoDocHandler
	implements
		IRequestHandler<GetPasajeroByNroDocAndTipoDocQuery, PasajeroDTO> {

	private final IPasajeroRepository _pasajeroRepository;

	public GetPasajeroByNroDocAndTipoDocHandler(
	...
	@Override
	public Future<PasajeroDTO> Handle(
		GetPasajeroByNroDocAndTipoDocQuery request
	) throws Exception {
		PasajeroDTO pasajeroDto = null;

		CompletableFuture<Pasajero> objPasajero = (CompletableFuture<Pasajero>) _pasajeroRepository.GetPasajeroByNroDocAndTipoDoc(
			request.getNroDoc(),
			request.getTipoDoc()
		);
