...
class MsReservaInfraestructureServiceTest {
	...
	void testMsReservaInfraestructureService() {
		MsReservaInfraestructureService msReservaInfraestructureService = new MsReservaInfraestructureService(
			service,
			_reservaRepository,
			_vueloRepository,
			_pasajeroRepository,
		...
		msReservaInfraestructureService.setReservaRepository(
			_reservaRepository
		);
		...
		assertEquals(
			_reservaRepository,
			msReservaInfraestructureService.getReservaRepository()
		);
		assertEquals(
			_vueloRepository,
			msReservaInfraestructureService.getVueloRepository()
		);
		assertEquals(
			_pasajeroRepository,
			msReservaInfraestructureService.getPasajeroRepository()
		);
		assertEquals(
			_unitOfWork,
			msReservaInfraestructureService.get_unitOfWork()
		);
	}
}
