...
class MemoryReservaRepositoryTest {
	...
	void testFindByIdAsync() throws Exception {
		...
		Future<Reserva> resultado = memoryReservaRepository.FindByIdAsync(
			reservaTest.getId()
		);
	...
	void testCreateAsync() throws Exception {
		when(_database.get_reservas()).thenReturn(lista);
		Future<Reserva> resultado = memoryReservaRepository.CreateAsync(
			reservaTest
		);
		assertNotNull(resultado.get());
	}
	...
	void testUpdateAsync() throws Exception {
		Future<Reserva> resultado = memoryReservaRepository.UpdateAsync(
			reservaTest
		);
	...
	void testGetReservasByHoraAndEstado() throws Exception {
		Future<List<Reserva>> resultado = memoryReservaRepository.GetReservasByHoraAndEstado(
			LocalDateTime.now(),
			"VALIDO"
		);
