...
public class VueloListenerEvent {
	...
	public void onApplicationEvent(DomainMessage event) {
		if (event.getMessage() instanceof Reserva) {
			try {
				System.out.println(
					"Se ha creado un evento de dominio de reserva"
				);
				Reserva reserva = (Reserva) event.getMessage();
				Future<Vuelo> future = vueloEntityRepository.FindByIdAsync(
					reserva.getVueloId()
				);
				Vuelo vuelo = future.get();
				if (
					vuelo.reducirDisponibilidad(
						reserva.getCantidadPasajero().getCantidad()
					)
				) {
				...
				System.out.println(
					"VueloListenerEvent->Ocurrio un error al procesar el evento" +
					e
				);
			}
		}
