...
@Repository
public interface PasajeroEntityRepository extends CrudRepository<PasajeroEntity, UUID>{
	
	@Query(value = "SELECT * FROM pasajero WHERE nrodoc = :nrodoc and tipodoc = :tipodoc limit 1", nativeQuery = true)
	Future<PasajeroEntity> GetPasajeroByNroDocAndTipoDoc(@Param("nrodoc") Integer nrodoc, @Param("tipodoc") Integer tipodoc);
