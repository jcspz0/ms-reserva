...
export class PasajeroService {
  ...
  getPasajeroByNroDocTipoDocBackEnd(nrodoc: number, tipodoc: number) {
    ...
    let queryParams = new HttpParams()
      .append('nrodoc', nrodoc)
      .append('tipodoc', tipodoc);

    //return
    let response = cloneDeep(
      this.httpget.get<Pasajero>(
        url,
        { params: queryParams }
  ...
  crearPasajeroBackEnd(pasajeroACrear: Pasajero) {
    const url =
      environment.apiHost + ':' + environment.apiPort + '/pasajero/crear';
    console.log('crear bk');
    console.log(pasajeroACrear);
    //let response = cloneDeep(this.httppost.post<Pasajero>(
    this.httppost.post<Pasajero>(url, pasajeroACrear).subscribe(
      (res) => {
        console.log(res);
