...
class ReservaControllerTest {
	...
	@InjectMocks
	ReservaController reservaControllerTest;
	...
	void CreateReserva() throws Exception {
		when(webApiServiceTest.getMediator().Send(any(GetVueloByIdQuery.class)))
			.thenReturn(vueloDTOTest);

		when(
			webApiServiceTest.getMediator().Send(any(CrearReservaCommand.class))
		)
			.thenReturn(UUID.fromString(reservaIdTest));

		ReservaDTO resultado = reservaControllerTest.CreateReserva(
			reservaDTOTest
		);
		...
		assertEquals(
			...
			resultado.getReservaId().toString()
		);
	}
	...
	void FindReservaById() throws Exception {
		when(webApiServiceTest.getMediator().Send(any()))
			.thenReturn(reservaDTOTest);

		ReservaDTO resultado = reservaControllerTest.FindReservaById(
			reservaIdTest
		);
