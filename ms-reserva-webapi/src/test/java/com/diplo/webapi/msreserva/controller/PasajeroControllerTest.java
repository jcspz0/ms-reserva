...
class PasajeroControllerTest {
	...
	@InjectMocks PasajeroController pasajeroControllerTest;
	...
	void init() {
		...
		PasajeroDTOTest = new PasajeroDTO(pasajeroIdTest.toString(), nroDocTest, tipoDocTest, nombreTest, primerApellidoTest, segundoApellidoTest);
	...
	void CrearPasajero() throws Exception {
		when(webApiServiceTest.getMediator().Send(any())).thenReturn(pasajeroIdTest);
		
		
		PasajeroDTO resultado = pasajeroControllerTest.CreatePasajero(PasajeroDTOTest);
		
		assertNotNull(resultado);
		assertEquals(pasajeroIdTest.toString(), resultado.getPasajeroId().toString());
	}
	...
	void EncontrarPasajeroPorNroDocYTipoDoc() throws Exception {
		...
		when(webApiServiceTest.getMediator().Send(any())).thenReturn(PasajeroDTOTest);
		
		PasajeroDTO resultado = pasajeroControllerTest.FindPasajeroByNroDocAndTipoDoc(nrodocTest, tipodocTest);
